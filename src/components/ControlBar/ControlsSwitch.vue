<template>
  <div class="controls-switch">
    <SvgButton :active="isPerspectiveView" @action="setPerspectiveView" class="connect-right" :title="$t('controls.perspective.tooltip')">
    ...
    <SvgButton :active="isFlatView" @action="setFlatView" class="connect-left" :title="$t('controls.flatView.tooltip')" 
               v-bind:class="{ 'connect-right': controls.freeFlightEnabled }">
      <svg viewBox="0 0 30 30">
        <path d="M22.371,4.158c1.65,0,3,1.35,3,3v15.684c0,1.65-1.35,3-3,3H7.629c-1.65,0-3-1.35-3-3V7.158c0-1.65,1.35-3,3-3H22.371z"/>
      </svg>
    </SvgButton>
    <SvgButton v-if="controls.freeFlightEnabled" :active="isFreeFlight" @action="setFreeFlight" class="connect-left" :title="$t('controls.freeFlight.tooltip')">
      <svg viewBox="0 0 30 30">
